class Slider {
  ...
  listeners() {
    ...
    window.addEventListener('touchend', (e) => {
      ...
      // You can adjust the sensitivity by changing the threshold (e.g., 50)
      if (Math.abs(deltaY) > 50) {
        // Determine the direction of the swipe
        const direction = deltaY > 0 ? 'down' : 'up';

        // Call the appropriate function based on the swipe direction
        if (direction === 'down') {
          this.prevSlide(); // You need to implement prevSlide() function
        } else {
          this.nextSlide();
        }
      }
